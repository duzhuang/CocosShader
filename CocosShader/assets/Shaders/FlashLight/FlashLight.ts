...
export default class FlashLight extends cc.Component {
    ...
    /**光束宽度 */
    @property({ type: cc.Float, tooltip: "光束宽度" })
    lightWidth = 0.2;

    /**时间 */
    @property({ type: cc.Float, tooltip: "时间" })
    LoopTime = 1.0;

    /**TimeInterval */
    @property({ type: cc.Float, tooltip: "TimeInterval" })
    TimeInterval = 3.0;
    ...
    update(dt: number) {
        ...
        this.dttime += dt;
        this.material.setProperty("lightCenterPoint", new cc.Vec2(this.time, this.time));          //设置材质对应的属性
